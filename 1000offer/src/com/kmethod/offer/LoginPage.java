...
public class LoginPage extends Activity {
	...
	private Handler m_handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case HttpClientUtil.NET_TIMEOUT:
				Toast.makeText(LoginPage.this, R.string.net_timeout,
						Toast.LENGTH_SHORT).show();
				break;
			case HttpClientUtil.NET_FAILED:
				Toast.makeText(LoginPage.this, R.string.net_failed,
						Toast.LENGTH_SHORT).show();
				break;
			default:
				break;
	...
	public void onLoginBtnClick(View v) {
		...
		if (uname.length() == 0) {
			Toast.makeText(this, R.string.login_username_alert,
					Toast.LENGTH_SHORT).show();
			return;
		}

		if (passwd.length() == 0) {
			Toast.makeText(this, R.string.login_passwd_alert,
					Toast.LENGTH_SHORT).show();
			return;
		}
			...
			public void execute(int status, String response,
				...
				if (status == HttpClientUtil.NET_SUCCESS) {
							...
							public void run() {
								...
								case Api.LOGIN_FAIL_LESS_THAN_FIVE:
									String alertText = "用户名或者密码错误,还有"
											+ (Api.ALLOW_LOGIN_USERNAME_OR_PASSWD_ERROR_NUM - retObj
													.getInteger("strikes"))
											+ "尝试机会";
									Toast.makeText(LoginPage.this, alertText,
											Toast.LENGTH_SHORT).show();
									break;
					...
					Api.getInstance().setLoginUserInfo(
							...
							retObj.getInteger("userid"),
							retObj.getInteger("isavatar"),
							retObj.getString("email"));

					for (int i = 0; i < cookies.size(); i++) {
						Cookie cookie = cookies.get(i);
						Api.getInstance().getCookieStorage()
								.addCookie(cookie.getName(), cookie.getValue());
					}
					LoginPage.this.sendBroadcast(new Intent(
