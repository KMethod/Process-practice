...
public class FeedbackPage extends Activity {
	...
	private Handler m_handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case HttpClientUtil.NET_TIMEOUT:
				Toast.makeText(FeedbackPage.this, R.string.net_timeout,
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		this.m_name
				.setText(Html.fromHtml(Api.getInstance().getLoginUserName()));
	...
	public void onSendBtnClick(View v) {
		...
		if (name.length() == 0) {
			Toast.makeText(FeedbackPage.this, "姓名不能为空", Toast.LENGTH_SHORT)
					.show();
		...
		if (email.length() == 0) {
			Toast.makeText(FeedbackPage.this, "Email不能为空", Toast.LENGTH_SHORT)
					.show();
			...
			if (!matcher.matches()) {
				Toast.makeText(FeedbackPage.this, "不是有效的Email地址",
						Toast.LENGTH_SHORT).show();
				this.m_email.requestFocus();
				return;
			}
		...
		if (msg.length() == 0) {
			Toast.makeText(FeedbackPage.this, "信息不能为空", Toast.LENGTH_SHORT)
					.show();
				...
				m_handler.post(new Runnable() {
					...
					public void run() {
						...
						case HttpClientUtil.NET_SUCCESS:
							if (response == null) {
								Toast.makeText(FeedbackPage.this, "提交反馈失败",
										Toast.LENGTH_SHORT).show();
								return;
							}
							...
							if (jsonObj == null) {
								Toast.makeText(FeedbackPage.this, "数据错误",
										Toast.LENGTH_SHORT).show();
								return;
							}
							...
							Toast.makeText(FeedbackPage.this, "提交反馈成功",
									Toast.LENGTH_SHORT).show();
